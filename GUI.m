...
% Last Modified by GUIDE v2.5 25-May-2016 16:05:34
...
global currentpoint;
global img_cashe;
img_cashe{1,currentpoint}.a=dst;
currentpoint=currentpoint+1;

% --- Executes on button press in pushbutton6.
function pushbutton6_Callback(hObject, eventdata, handles)%撤销
global img_cashe;
global img_dst;
global currentpoint;
if currentpoint==2
    ...
    img_dst=img_cashe{1,currentpoint}.a;
    save(img_dst);
end;
...
%y=imhist(x); %直方图
level = graythresh(x);
img_dst=im2bw(x,level);  % im2bw为MATLAB把图象转化为二值图象的函数 
save(img_dst);
